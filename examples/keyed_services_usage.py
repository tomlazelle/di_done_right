...
class PaymentProcessor:
    ...
    def process_payment(self, payment_method: str, amount: float) -> str:
        ...
        if payment_method in ["paypal", "stripe"]:
            notification_service = self.container.resolve_keyed(INotificationService, "email")
        else:
            notification_service = self.container.resolve_keyed(INotificationService, "sms")
...
def main():
    ...
    container.register_keyed(IPaymentService, "paypal", PayPalService, ServiceLifetime.TRANSIENT)
    container.register_keyed(IPaymentService, "stripe", StripeService, ServiceLifetime.TRANSIENT)
    ...
    container.register_keyed(INotificationService, "email", EmailNotificationService, ServiceLifetime.TRANSIENT)
    container.register_keyed(INotificationService, "sms", SmsNotificationService, ServiceLifetime.TRANSIENT)
