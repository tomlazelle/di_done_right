...
class SqlOrderRepository(IOrderRepository):
    ...
    def update_order_status(self, order_id: int, status: OrderStatus) -> None:
        if order_id in self.orders:
            ...
            self.logger.log("INFO", f"Updated order {order_id} status to {status.value}")
...
class OrderService(IOrderService):
    ...
    def process_order(self, order_id: int, payment_method: str) -> bool:
        ...
        # Process payment
        payment_result = payment_service.process_payment(order.total_amount, payment_method)
...
def configure_services(container):
    ...
    container.register_keyed(IPaymentService, "stripe", StripePaymentService, ServiceLifetime.SINGLETON)
    container.register_keyed(IPaymentService, "paypal", PayPalPaymentService, ServiceLifetime.SINGLETON)
    
    # Notification services (Keyed - multiple implementations)
    container.register_keyed(INotificationService, "email", EmailNotificationService, ServiceLifetime.SINGLETON)
    container.register_keyed(INotificationService, "sms", SmsNotificationService, ServiceLifetime.SINGLETON)
...
def demo_keyed_services():
    ...
    for notification_type in ["email", "sms"]:
        notification_service = ServiceProvider.resolve_keyed(INotificationService, notification_type)
...
def create_fastapi_app():
    ...
    try:
        ...
        from di_container.fastapi_integration import FastAPIIntegration, inject, inject_keyed
